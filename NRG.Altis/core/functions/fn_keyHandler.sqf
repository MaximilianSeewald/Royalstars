...
switch (_code) do {
    ...
    case 19: {
		...
		if !(license_civ_rebel) exitWith { hintSilent "Du benötigst eine Rebellenausbildung um jemanden zu fesseln!"; };
		if(_shift && playerSide == civilian && !isNull cursorTarget && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && (side cursorTarget in [civilian]) && alive cursorTarget && cursorTarget distance player < 3.5 && !(cursorTarget getVariable "Escorting") && !(cursorTarget getVariable "restrained") && speed cursorTarget < 1) then {
